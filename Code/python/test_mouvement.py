...
for i in range(nb_series):
    for j in range(nb_photo-1):
        ...
        image_right = cv.imread('/home/pi/Documents/TD/photo/samba/photo_diff/subtract/diff_avec_rotation_avec_perte/serie{0}'.format(i+1) + '_image{0}'.format(j) +'_diff_right_binary.jpg', cv.COLOR_BGR2GRAY)
        ...
        pourcent_per_image_left = 0
        # load the two input image_rights
        image_left = cv.imread('/home/pi/Documents/TD/photo/samba/photo_diff/subtract/diff_avec_rotation_avec_perte/serie{0}'.format(i+1) + '_image{0}'.format(j) +'_diff_left_binary.jpg', cv.COLOR_BGR2GRAY)
